...
source("tikz.R")
source("colors.R")
tempLines <- readLines("sample-level-curves-template.tex")
...
tex <- ""
for(Nsamp in names(simulation.samples$data)){
  Nseed <- 2
...
print(p)
    ...
    out <- sprintf("figure-norm-level-curves-%s-%s.tex", norm, Nsamp)
    print(out)
    tikz(out, w=5, h=3)
    print(p)
    dev.off()

    tex <- paste(tex, sprintf(template, Nsamp, norm.to.tex[norm], out))
  }
}

writeLines(tex, "sample-level-curves.tex")
